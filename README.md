...
- 現在または1995年時点の自治体、島221件、湖・池92件を検索して、ナウルと面積・輪郭を比較
- マイ地点（最大2件）
...
本サイトはスケール感を体験するシミュレーターです。現在の市区町村・都道府県と、島・湖／池の大部分の面積には国土地理院の公表値を使用しています。1995年時点の自治体と公表値を紐づけられない一部の島・湖／池は、境界データから算出した概算です。島・湖／池は国土地理院の1km²以上一覧を基準に収録しています。シルエットの見た目やメルカトル図法上の表示は、厳密な測量には使用できません。
